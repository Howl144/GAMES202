...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    Intersection inter;
    //方向与法线夹角大于90度，则判定为不相交
    if (dotProduct(ray.direction, normal) > 0)
        return inter;
    //Moller Trumbore算法
    //O + tD = (1 - u - v) * v0 + u * v1 + v * v2
    //O - V0 = (V1 - V0)u + (V2 - V0)v - tD
    //     S =     E1 * u +     E2 * v - tD
    //[-D, E1, E2] * [t, u, v]^T = S
    //克拉默法则，形如A * x = c 的线性方程组,xc为列向量。
    //t = det(S, E1, E2) / det(-D, E1, E2)
    //u = det(-D, S, E2) / det(-D, E1, E2)
    //v = det(-D, E1, S) / det(-D, E1, E2)
    //混合积可以得出                            ↓优化后的式子，减少计算次数
    //denom = det(-D,E1,E2) = -D * (E1 × E2) = E1 * (D × E2) = E1 * S1
    //tnom  = det(S,E1,E2) =  S * (E1 × E2)  = E2 * (S × E1) = E2 * S2
    //unom  = det(-D,S,E2) = -D * (S × E2)   = S  * (D × E2) = S  * S1
    //vnom  = det(-D,E1,S) = -D * (E1 × S)   = D  * (S × E1) = D  * S2
    double u, v, t_tmp = 0;
    //S1 = D × E2
