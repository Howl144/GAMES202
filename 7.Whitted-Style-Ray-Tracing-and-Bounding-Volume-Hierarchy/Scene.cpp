    ...
    if(intersection.happened) {
        ...
        switch (m->getType()) {
            case REFLECTION_AND_REFRACTION:
            {
                ...
                //epsilon是为了区分反射光线和折射光线的起点在物体内部还是外部，如果不加这epsilon，由于隐式表面求交会得到两个解析解，当发射光线在物体表面时，会出现求交始终得到t等于0(趋近于0)的情况，这样就会一直递归下去，导致该像素为黑色。
            ...
            default:
            {
                ...
                // [/comment]
                Vector3f lightAmbient = 0.1 * hitObject->evalDiffuseColor(st); 
                Vector3f lightDiffuse = 0, specularColor = 0;
                ...
                for (uint32_t i = 0; i < get_lights().size(); ++i)
                {
                    ...
                    else
                    {
                        ...
                        //diffuse
                        Vector3f dif = (1 - inShadow) * get_lights()[i]->intensity * LdotN;
                        lightDiffuse += dif * hitObject->evalDiffuseColor(st);
                        //specular
                ...
                hitColor = lightAmbient + lightDiffuse  * m->Kd + specularColor * m->Ks;
                break;
            }
        }
    }
